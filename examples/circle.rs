...
impl CircleScenario {
    pub fn new(name: &str) -> Self {
        ...
        let RADIUS = args[5].parse::<f64>().unwrap();
        let RES = args[6].parse::<usize>().unwrap();
        let DAMP = args[7].parse::<f64>().unwrap();

        let ACTIVE_SET_EPI = args[8].parse::<f64>().unwrap();
        let NEIGHBOR_LEVEL = args[9].parse::<usize>().unwrap();

        let FILENAME = &args[10];
        let COMMENT = &args[11];
        let TOTAL_FRAME = args[12].parse::<usize>().unwrap();
        let MODIFICATION = &args[13];

        let precision = args[14].parse::<usize>().unwrap();
        ...
        let mut p = circle(RADIUS, RES, Some(DENSITY));
...
fn main() {
    ...
    let problem = CircleScenario::new("beamnew");
